...
dependencies {
    compile 'info.picocli:picocli:4.1.4'
    compileOnly 'info.picocli:picocli-codegen:4.1.4'
}
```
...
* `disable.resources.config` - disable the <<ResourceConfigGenerator>> tool so no  `resources-config.json` file is generated
...
Once we generated the template files, we will need to tell the `asciidoctor` tool to take our template files as input instead of the generated manpage AsciiDoc files.
