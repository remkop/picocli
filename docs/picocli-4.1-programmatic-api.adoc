...
public class ProgrammaticCommand {
    ...
    public static void main(String[] args) {
        ...
        spec.addOption(OptionSpec.builder("-c", "--count").paramLabel("COUNT").type(int.class)
                .description("number of times to execute").build());
        spec.addPositional(PositionalParamSpec.builder().paramLabel("FILES").type(List.class).auxiliaryTypes(File.class) // List<File>
                .description("The files to process").build());
        CommandLine commandLine = new CommandLine(spec);
        
        // set the execution strategy and control other aspects,
        // like streams and handlers to be used and exit codes
...
// or delegated to a configured execution strategy.
class Strategy implements IExecutionStrategy {

    @Override
    public int execute(ParseResult pr) throws ExecutionException, ParameterException {
...
The `CommandLine` class has an `execute` convenience instance method which allows to reduce some boilerplate when executing the command programmatically.
Applications can do configuration of the execution before invoking the command.
For example:
