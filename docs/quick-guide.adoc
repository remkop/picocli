...
=== Basic example application: ASCIIArt
...
----
$ java -cp "myapp.jar;picocli-4.5.1.jar" ASCIIArt --font=speed Hello picocli
...
=== Example application with subcommands: ISO Code resolver
Below we show another small but fully functional example picocli-based command line application which explains the use of subcommands: `ISO Code resolver`.

The `ISOCodeResolver` application has two subcommands `language` and `country` that may be used to either resolve either language or country code(s) following ISO standard. The app can be used as follows:

.Resolving two letter language codes
----
$ java -cp "myapp.jar;picocli-4.5.1.jar" ISOCodeResolver language de cs en sd se
de: German
cs: Czech
en: English
sd: Sindhi
...
----
$ java -cp "myapp.jar;picocli-4.5.1.jar" ISOCodeResolver country cn fr th ro no
CN: China
FR: France
TH: Thailand
RO: Romania
...
The source code of our second `ISOCodeResolver` example is also fairly small:
...
@Command(name = "ISOCodeResolver", subcommands = { SubcommandAsClass.class, CommandLine.HelpCommand.class }, // <2>
         description = "Resolve ISO country codes (ISO-3166-1) or language codes (ISO 639-1/-2)")
public class ISOCodeResolver implements Runnable { // <1>
    @Spec CommandSpec spec;

    @Command(name = "country", description = "Resolve ISO country code (ISO-3166-1)") // <3>
    void subCommandViaMethod(@Parameters(arity = "1..*", paramLabel = "<code 1> <code 2>",
...
@Command(name = "language", description = "Resolve ISO language code (ISO 639-1 or -2)") // <4>
class SubcommandAsClass implements Runnable {

    @Parameters(arity = "1..*", paramLabel = "<code> <code2>", description = "language code(s)")
...
<1> Create a class that implements `Runnable` or `Callable`. This is your main command.
<2> Annotate the class with `@Command` and give it a name. By specifying the https://picocli.info/apidocs/picocli/CommandLine.HelpCommand.html[CommandLine.HelpCommand] class as subcommand inside the annotation, we can add the pre-built `help` subcommand to your application.
<3> Custom subcommands may be added you main command in two different ways. The easiest way is to add an `@Command`-annotated method to your main command class. If your subcommand can take options and/or positional parameters, you have to annotate the method's argument variable with `@Option` and/or `@Parameters` annotations. In the example above, once the user invokes the subcommand `country`, the associated method `subCommandViaMethod` gets called.
<4> In larger applications, it is common to code subcommands inside their own `@Command`-annotated class. In the example above, the `SubcommandAsClass` class represents the `language` subcommand. Once the user invokes this subcommand, the overridden method `run` of this class gets called. To be recognized as subcommand by the main app, the subcommand class must be given in the `subcommands` attribute of the `@Command@` annotation of the main class (subcommands = { SubcommandAsClass.class, ... } ❷).
<5> In the `main` method of your main `ÌSOCodeResolver` class, we use the `CommandLine.execute` method to bootstrap our application.
...
<6> The `CommandLine.execute` method returns an exit code. Your application can call `System.exit` with this exit code to signal success or failure to the calling process.
...
In addition to the two user defined subcommands, the `ISOCodeResolver` app offers a `help` subcommand, which will show usage instructions to the user.
Picocli generates this usage help message automatically:
...
Resolve ISO country codes (ISO-3166-1) or language codes (ISO 639-1 or -2)
Commands:
  help      Displays help information about the specified command
  country   Resolve ISO country code (ISO-3166-1, Alpha-2 code)
  language  Resolve ISO language code (ISO 639-1 or -2, two/three letters)
----
