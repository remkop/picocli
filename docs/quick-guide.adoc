...
<3> Custom subcommands can be added to the top-level command in two ways.
    ...
    For each option and positional parameter of the subcommand, add a method argument, and annotate these method argument with the `@Option` or `@Parameters` annotation.
