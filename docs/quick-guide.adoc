...
The `ISOCodeResolver` application has two subcommands, `language` and `country`, that may be used to either resolve either languages or country codes following the ISO standards (ISO-3166-1 for country codes, and ISO 639-1/639-2 for language codes).
...
<1> When the main command does not implement `Runnable` or `Callable`, users must specify a subcommand (subcommands become mandatory).
