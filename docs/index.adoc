...
Examples of commonly used ways to provide a password that avoid specifying it in plain text on the command line: `--password:file=$HOME/mypasswordfile` (security based on file permissioning) or `--password:env=ENV-KEY` where the application - not the shell! - gets the value from the user’s environment variables (security based on OS login). 
