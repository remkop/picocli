...
* `java.util.UUID`
* `java.lang.Class` (from picocli 2.2)
* `java.nio.ByteOrder` (from picocli 2.2)
* `java.util.Currency` (from picocli 2.2)
* `java.util.TimeZoneConverter` (from picocli 2.2)
* `java.nio.file.Path` (from picocli 2.2, requires Java 7 or higher)
* `java.time` value objects: `Duration`, `Instant`, `LocalDate`, `LocalDateTime`, `LocalTime`, `MonthDay`, `OffsetDateTime`, `OffsetTime`, `Period`, `Year`, `YearMonth`, `ZonedDateTime`, `ZoneId`, `ZoneOffset`  (from picocli 2.2, requires Java 8 or higher)
...
NOTE: Picocli works with Java 5 and higher, but it does have some default converters for Java 7 types like `Path` and Java 8 types like `Duration`, etc. These converters are only available if the platform supports them. See the above list for details. In addition, Java 8 lambdas make it easy to register custom converters for additional types:

[source,java]
----
commandLine.registerConverter(Locale.class, s -> new Locale.Builder().setLanguageTag(s).build());
commandLine.registerConverter(Cipher.class, s -> Cipher.getInstance(s));
----
...
CommandLine commandLine = new CommandLine(app)
        .registerConverter(Locale.class, s -> Locale.Builder().setLanguageTag(s).build())
...
public static void main(String... args) {
    ...
    // declaratively registered the subcommands via annotation)
...
[source,groovy]
----
@Grab('info.picocli:picocli:2.2.0-SNAPSHOT')
...
=== Gradle
----
compile 'info.picocli:picocli:2.2.0-SNAPSHOT'
----
...
=== Scala SBT
----
libraryDependencies += "info.picocli" % "picocli" % "2.2.0-SNAPSHOT"
----

=== Ivy

----
<dependency org="info.picocli" name="picocli" rev="2.2.0-SNAPSHOT" />
----
