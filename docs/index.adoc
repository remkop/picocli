...
=== `@Inject` annotation
Picocli 3.0 introduces an `@Inject` annotation for injecting picocli API objects into command fields.
...
Currently only `CommandLine`, `CommandSpec` and `ParseResult` objects can be injected.
