...
TIP: Interactive options will cause the application to wait for input on stdin. If your command also needs to be run in (non-interactive) batch mode, it should provide additional non-interactive alternative options to allow end users to run the command interactively as well as in batch mode.
In the above example, a `--password:file` option that takes a `File` or `Path` parameter could be added where the application reads the password from a file.
Another idea is to add a `--password:env` option that takes an environment variable name parameter, where gets the application get the password from the user’s environment variables.
Combining either of these with an interactive `--password` option gives users a way to provide a password without specifying it in plain text on the command line, both interactively and in batch mode.
...
[source,groovy]
----
@Grab('info.picocli:picocli:3.6.0-SNAPSHOT')
...
=== Gradle
----
compile 'info.picocli:picocli:3.6.0-SNAPSHOT'
----
...
=== Scala SBT
----
libraryDependencies += "info.picocli" % "picocli" % "3.6.0-SNAPSHOT"
----

=== Ivy

----
<dependency org="info.picocli" name="picocli" rev="3.6.0-SNAPSHOT" />
----
