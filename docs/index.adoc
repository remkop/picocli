...
Sometimes you need to write a Mixin class that accesses the mixee command where it is mixed in.

This can be accomplished with the <<spec-annotation, `@Spec` annotation>>, by setting its target to `Spec.Target.MIXEE`.
For example:
...
class AdvancedMixin {
    ...
    @Option(names = "-x")
    void setValue(int x) {
        // get another value from the command we are mixed in to
...
=== Use Case: Global Option to Configure Log Level
