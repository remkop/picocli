...
Try it without arguments, with an option like `--help` or `--version`, or with a file name like `/usr/bin/java` or `/proc/cpuinfo` as command line argument.
====
...
@Command(name = "checksum", mixinStandardHelpOptions = true, version = ["checksum 4.5.1"],
    description = ["Prints the checksum (MD5 by default) of a file to STDOUT."])
...
After we successfully compiled our <<Checksum-App-Java,example `CheckSum` application>>, let's quickly look at how to run it.
