...
Options with `arity = "0..1"` have an optional parameter.
This can be useful for <<Optionally Interactive,password options>> and in some other cases.
...
class Ambiguous {
    ...
    @Option(names = "-y", arity = "0..*",
      description = "Option with one or more parameters")
    List<String> values;
}
----

When `-x VALUE` is specified on the command line, this results in an error: `Missing required parameter: <file>`. Similarly, when `-y a b c path/to/file` is specified.

Users can disambiguate the input with the <<Double dash (`--`),end-of-options delimiter>>,
and specify `-x &dash;&dash; VALUE` or `-y a b c &dash;&dash; VALUE`, but it may be better to redesign your command to avoid the ambiguity.
====
