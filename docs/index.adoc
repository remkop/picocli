...
public class Example {
    ...
    @Parameters(arity = "1..*", valueLabel = "FILE", description = "File(s) to process.")
...
class Convert {
    @Option(type = java.util.regex.Pattern.class, // List elements are Pattern objects
            names = "-patterns", valueLabel = "PATTERN",
    ...
    @Parameters(type = File.class, // convert arguments to File before adding to the List
                valueLabel = "FILE", description = "files to convert")
    List<File> files;
}
----
...
[source,java]
----
commandLine.registerConverter(Byte.class,    new ITypeConverter<Byte>()   { public Byte    convert(String s) {return Byte.decode(s);}});
commandLine.registerConverter(Short.class,   new ITypeConverter<Short>()  { public Short   convert(String s) {return Short.decode(s);}});
commandLine.registerConverter(Integer.class, new ITypeConverter<Integer>(){ public Integer convert(String s) {return Integer.decode(s);}});
commandLine.registerConverter(Long.class,    new ITypeConverter<Long>()   { public Long    convert(String s) {return Long.decode(s);}});
----
