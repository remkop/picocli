...
----
=== Parameter Labels
Option parameters and positional parameters are shown in the synopsis and the options list by the name of the annotated field in `<` and `>` fish brackets.
Use the `paramLabel` attribute to display a different name. For example:
[source,java]
----
@Option(names = "-f", paramLabel = "FILE") File f;
@Option(names = "-n") int number;
@Parameters(index = "0", paramLabel = "NUM") int n;
@Parameters(index = "1") InetAddress host;
----
