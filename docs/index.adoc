...
From picocli 4.3, picocli makes it easy to accomplish this: simply make the top-level command a class that does not implement `Runnable` or `Callable`.

When the <<Executing Subcommands,`execute` method>> is used and picocli detects that the top-level command does not implement `Runnable` or `Callable`, picocli internally throws a `ParameterException` with the error message `"Missing required subcommand"`. This will cause the <<Invalid User Input, `IParameterExceptionHandler`>> to be called, which will display this message to the user.
