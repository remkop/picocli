...
After registering custom converters, call the `parseCommands(String...)` method on the `CommandLine` instance where the converters are registered. (The static `parse` method cannot be used.) For example:
...
commandLine.parseCommands("-d", "PT15M", "file1.txt");
...
=== Parsing Subcommands
The `CommandLine.parseCommands` method returns a List with the recognized commands. The main command (`git` in this example) is always the first element in the returned list.
...
commandLine.parseCommands("-f:output.txt");
assert optionArg.file.equals("output.txt");
----
