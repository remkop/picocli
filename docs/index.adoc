...
The `CommandLine.call` method is a convenience method that parses the command line arguments and
only invokes the `Callable` when the command line parameters are valid.
Otherwise it shows the usage help message.
For example, when this application is run without any parameters, the following usage help message is displayed:
...
Usage: checksum [-h] [-a=<algorithm>] <file>
...
==== Maps
Picocli supports `java.util.Map` fields since 1.0.0.

For `Map` fields, use the `type` attribute to specify the both the _key type_ and the _value type_. For example:
...
----
For map options, the same option name may be specified multiple times with different key-value pairs.
