...
== Reuse
Your application may have options or usage help attributes that are common to all commands.
To reduce duplication you may want to reuse such options and attributes. Picocli supports both inheritance and composition for reuse.

Let's take some example options and attributes and see how to reuse them. For example:
...
All commands that extend `ReusableOptions` support the `--help` and `--version` options, and generate a usage help message in the same spacious style. For example:
...
=== Programmatic Mixin
Picocli 3.0 introduces the concept of "mixins". Mixins are a convenient alternative to subclassing. Common options, common positional parameters or common usage help attributes can be defined in a separate class. This class can be reused in many other commands, either programmatically with the `CommandLine.addMixin` method, or declaratively with the `@Mixin` annotation.
...
CommonOptionsAndAttributes mixin = new ReusableOptions();
...
assert commandLine.isUsageHelpRequested();
assert mixin.helpRequested;
----

=== `@Mixin` Annotation
A command can include a mixin by annotating a field with `@Mixin`. All picocli annotations of the mixin are added to the command that declares a `@Mixin` field. For example:
...
----
When using the `@Mixin` annotation, the command can reference the mixed-in object directly (for example, the `MyCommand` can reference its `myMixin` field to access the `ReusableOptions` object.
...
Mixins can be nested. There is no limitation to how deeply mixins can be nested. A mixin may also inherit options, positional parameters and command attributes from a super class.
...
Command attributes may be defined multiple times, but only one value is preserved. In order of priority:

. @Command attributes of the command itself
. Attributes on the @Mixin commands
. Attributes on nested @Mixin of another @Mixin
