...
== Multiple Values
Multi-valued options and positional parameters can capture multiple values from the command line.
...
Every time `@Parameters` field's index matches the index of the positional parameter, the field may consume zero, one or more arguments, depending on its arity.
...
// TODO update documentation for convenience methods
...
----
The `CommandLine.call` method returns the result of the `Callable`, or `null` if the command line options were invalid. An error message and a usage help message are printed when the command line options were invalid. Exceptions thrown from the `Callable.call` method are not caught by the `CommandLine.call` method.
...
CAUTION: Note that the `CommandLine.call` and `CommandLine.run` convenience methods cannot be used with subcommands.
...
=== Gradle
----
compile 'info.picocli:picocli:2.0.0-SNAPSHOT'
----
...
=== Scala SBT
----
libraryDependencies += "info.picocli" % "picocli" % "2.0.0-SNAPSHOT"
----

=== Ivy

----
<dependency org="info.picocli" name="picocli" rev="2.0.0-SNAPSHOT" />
----
