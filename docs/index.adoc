...
[source,bash]
----
java -cp "picocli-4.5.2.jar:checksum.jar" CheckSum --algorithm SHA-1 hello.txt
----
...
class Bar : Callable<Int> {
    ...
    @Command(name = "baz", description = ["I'm a subcommand of `bar`"])
    fun baz( @CommandLine.Option(names = ["-z"]) z : Int) : Int {
...
[source,bash]
----
java -cp "picocli-4.5.2.jar;myapp.jar" org.myorg.MyMainClass --option=value arg0 arg1
----
...
[source,bash]
----
alias mycommand='java -cp "/path/to/picocli-4.5.2.jar:/path/to/myapp.jar" org.myorg.MainClass'
----
...
----
path/to/native-image -cp picocli-4.5.2.jar --static -jar myapp.jar
----
...
Scripts should use `@Grab('info.picocli:picocli-groovy:4.5.2')`. The old artifact id `@Grab('info.picocli:picocli:4.5.2')` will not work,
