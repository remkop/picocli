...
* If the parser is configured to <<Toggle Boolean Flags,toggle boolean options>>, the opposite of the _current value_ is assigned.
...
The default values of options in an argument group are applied to the options when picocli instantiates the user object of the group.
Picocli instantiates the user object of the group when at least one option in the group is matched on the command line.

Picocli will not initialize the `@ArgGroup`-annotated field
if none of the group options is specified on the command line.
For optional groups (groups with `multiplicity = "0..1"`) this means that the `@ArgGroup`-annotated field may remain `null`.

==== Default Values in Group Usage Help

The `${DEFAULT-VALUE}` variable can be used in the description of options in an argument group,
 but with one limitation: only default values that are defined via the `@Option(defaultValue = "...")` annotations will be shown in the usage help message. For example:

[source,java]
----
// this works correctly: usage help shows the default value
...
[source,java]
----
// this does not work correctly: usage help shows zero as the default value
...
Some examples for customizing the help are https://github.com/remkop/picocli/tree/master/picocli-examples/src/main/java/picocli/examples/customhelp[here].
...
[source,groovy]
----
@Grab('info.picocli:picocli-groovy:4.0.5-SNAPSHOT')
...
Scripts should use `@Grab('info.picocli:picocli-groovy:4.0.5-SNAPSHOT')`. The old artifact id `@Grab('info.picocli:picocli:4.0.5-SNAPSHOT')` will not work,
...
=== Scala SBT
----
libraryDependencies += "info.picocli" % "picocli" % "4.0.5-SNAPSHOT"
----

=== Ivy

----
<dependency org="info.picocli" name="picocli" rev="4.0.5-SNAPSHOT" />
----
