...
and generating rich usage help message.

To let users run your application without requiring picocli as an external dependency,
picocli is only a single source file, so you can include it _in source form_ in your application.
...
.Pre-defined styles and colors that can be used in descriptions and headers using the `@|` notation
