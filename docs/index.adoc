...
The user manual for the https://picocli.info/man/3.x/[latest stable release (3.9.6) is here].
...
Then invoke `CommandLine.parseArgs` or `CommandLine.populateCommand` with the command line parameters and an object you want to initialize.
...
A command can be <<Executing Commands,executed>> in one line of code. The example below uses the `CommandLine.execute` method
...
The <<Mixin Standard Help Options,mixinStandardHelpOptions>> attribute is all
...
For Java JVM-style options, the `:+` is turned into `:-` and vice versa.
...
After registering custom converters, call the `parseArgs(String...)` method on the `CommandLine` instance where the converters are registered. (The static `populateCommand` method cannot be used.) For example:
...
WARNING: From picocli 4.0, the `@picocli.groovy.PicocliScript` annotation class and supporting classes have been moved into a separate module, `picocli-groovy`. Use `@Grab('info.picocli:picocli-groovy:4.0.0-beta-1-SNAPSHOT')` from version 4.0, since `@Grab('info.picocli:picocli:4.0.0-beta-1-SNAPSHOT')` will not work.
...
=== Gradle
----
compile 'info.picocli:picocli:4.0.0-beta-1-SNAPSHOT'
----
...
=== Scala SBT
----
libraryDependencies += "info.picocli" % "picocli" % "4.0.0-beta-1-SNAPSHOT"
----

=== Ivy

----
<dependency org="info.picocli" name="picocli" rev="4.0.0-beta-1-SNAPSHOT" />
----
