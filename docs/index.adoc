...
=== Subclassing
One way to reuse the above option and attributes is to extend the class.
...
=== Mixins
Picocli 3.0 introduced the concept of "mixins". Mixins are a convenient alternative to subclassing:
...
A command receiving these mixed in options, positional parameters, and command attributes is called a "mixee".
...
Mixins can be installed programmatically by calling the `CommandLine.addMixin` method with an instance of the mixin class.
...
Similarly, a <<Subcommands as Methods,`@Command`-annotated method>> can include a mixin by declaring a `@Mixin`-annotated parameter.
All picocli annotations found in the mixin class are added to the command (the "mixee").
For example:

[source,java]
----
@Command(name = "mymixee", description = "Example reuse with @Mixin annotation.")
public class MyCommand {
    ...
    @Command
    void subzip(@Mixin ReusableOptions opts) {
        // ...
    }
}
----
...
Since picocli 4.2, <<_parentcommand_annotation,`@ParentCommand`-annotated>> fields can be used in mixins, which makes this easier. Here is an example implementation:
...
class MyMixin {
    ...
    @Option(names = "-v")
    public void setVerbose(boolean[] verbosity) { top.setVerbose(verbosity); }
}

// An example Top-level command with two subcommands.
// The `-v` option can be specified on the top-level command as well as the subcommands.
@Command(subcommands = Sub2.class)
class Top implements Runnable {
    private boolean[] verbosity = new boolean[0];

    @Option(names = {"-v", "--verbose"})
    public void setVerbose(boolean[] verbosity) { this.verbosity = verbosity; }
    ...
    // The @Mixin-annotated parameter adds the `-v` option to this subcommand.
    @Command
    void sub1(@Mixin MyMixin mymixin) { verbose("Hello from sub1%n"); }
}
...
class Sub2 implements Runnable{
    ...
    @Override
    public void run() { mymixin.top.verbose("Hello from sub2%n"); }
}
----
