...
To reduce duplication, picocli supports both inheritance and composition as ways to reuse such options and attributes.

Let's take a look at an example for how both mechanisms for reuse work. The first step is to extract these options, parameters and command attributes into a separate class. For example:
...
All commands that extend `ReusableOptions` support the `--verbose` option, and generate a usage help message in the same spacious style. For example:
...
=== Programmatic Mixin
Picocli 3.0 introduces the concept of "mixins". Mixins are a convenient alternative to subclassing. Common options, positional parameters or command attributes defined in a separate class can be reused in other commands, either programmatically with the `CommandLine.addMixin` method, or declaratively with the `@Mixin` annotation.
...
=== `@Mixin` Annotation
From picocli 3.0, a command can include a mixin by annotating a field with `@Mixin`. All picocli annotations found in the mixin class are added to the command that has a field annotated with `@Mixin`. For example:
