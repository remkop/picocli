...
==== Gradle
----
compile 'info.picocli:picocli:4.0.2-SNAPSHOT'
----
...
dependencies {
    compile 'info.picocli:picocli:4.0.2-SNAPSHOT'
    annotationProcessor 'info.picocli:picocli-codegen:4.0.2-SNAPSHOT'
}
```
...
====
Interactive options do not work in conjuction with JLine 2's `ConsoleReader`. Either implement a `IParameterConsumer` which uses `ConsoleReader` directly or use `picocli-shell-jline3`.
====
...
[source,groovy]
----
@Grab('info.picocli:picocli-groovy:4.0.2-SNAPSHOT')
...
Scripts should use `@Grab('info.picocli:picocli-groovy:4.0.2-SNAPSHOT')`. The old artifact id `@Grab('info.picocli:picocli:4.0.2-SNAPSHOT')` will not work,
...
=== Scala SBT
----
libraryDependencies += "info.picocli" % "picocli" % "4.0.2-SNAPSHOT"
----

=== Ivy

----
<dependency org="info.picocli" name="picocli" rev="4.0.2-SNAPSHOT" />
----
