...
From picocli 4.2, it is possible to mark a command as `@Command(subcommandsRepeatable = true)`.

Normally (`subcommandsRepeatable` is `false`), only one subcommand is matched, and potentially one sub-subcommand of that subcommand, etc. In mathematical terms, a hierarchy of subcommands can be represented by a _directed rooted tree_, illustrated by the diagram below.

image::subcommands-non-repeatable.png[]

When `subcommandsRepeatable` is set to `true`, the subcommands of this command may appear multiple times.
This allows users to specify a single subcommand repeatedly, but also to specify a sequence of sibling subcommands. The red arrows in the diagram below illustrate the additional sequences that are allowed when a command is repeatable.

image::subcommands-repeatable.png[]

An example use case is TODO. For example:
...
myapp add -x=item1 -w=0.2 \
      add -x=item2 -w=0.5 \
      list --where "w>0.2" \
      send-report --to=recipients@myorg.com
----
...
[source,groovy]
----
@Grab('info.picocli:picocli-groovy:4.2.0-SNAPSHOT')
...
Scripts should use `@Grab('info.picocli:picocli-groovy:4.2.0-SNAPSHOT')`. The old artifact id `@Grab('info.picocli:picocli:4.2.0-SNAPSHOT')` will not work,
...
=== Scala SBT
----
libraryDependencies += "info.picocli" % "picocli" % "4.2.0-SNAPSHOT"
----

=== Ivy

----
<dependency org="info.picocli" name="picocli" rev="4.2.0-SNAPSHOT" />
----
