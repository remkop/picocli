...
The `index` attribute accepts _range_ values, so an annotation like `@Parameters(index="2..4")` captures arguments at index 2, 3 and 4. Range values can be _open-ended_. For example, `@Parameters(index="3..*")` captures all arguments from index 3 and up.
...
assert demo.params != null && demo.params.length == 3;
...
* You can decide to force disable or force enable ANSI escape codes programmatically with the `CommandLine.ansi` static field. This overrides the value of system property `picocli.ansi`. For example:
[source,java]
----
