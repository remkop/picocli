...
Command line arguments starting with the `@` character are taken as the path to a file containing argument strings.
When this file is expanded, the `@file` argument is replaced by a list of arguments,
one argument for each line in the file. Lines starting with `#` are comments and are ignored.
Multiple @-files may be specified on the command line.
The specified path may be relative or absolute.

For example, suppose a file with arguments exists at `/home/foo/args`, with these contests:
----
...
=== Gradle
----
compile 'info.picocli:picocli:2.1.0-SNAPSHOT'
----
...
=== Scala SBT
----
libraryDependencies += "info.picocli" % "picocli" % "2.1.0-SNAPSHOT"
----

=== Ivy

----
<dependency org="info.picocli" name="picocli" rev="2.1.0-SNAPSHOT" />
----
