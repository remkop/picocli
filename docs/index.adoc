...
Picocli 4.0 introduces a new `execute` method to execute commands. This replaces the older `run`, `call`, `invoke` and `parseWithHandlers` methods. 
...
==== Migrating From `call` to `execute`
...
==== Migrating From `invoke` to `execute`
...
==== Migrating From `parseWithHandlers` to `execute`
...
The `IParseResultHandler2` has been replaced with `IExecutionStrategy`. The existing built-in handlers `RunLast`, `RunAll` and `RunFirst` can still be used:

* the `RunLast` handler prints help if requested, and otherwise gets the last specified command or subcommand and tries to execute it as a `Runnable`, `Callable` or `Method`
...
Object statusResult = cmd.getSubcommands().get("status").getExecutionResult();
----
...
This will usually look something like the below:
...
try {
    ...
    cmd.setExecutionResult(callable.call());
...
[source,groovy]
----
@Grab('info.picocli:picocli:4.0.0-alpha-3-SNAPSHOT')
...
=== Gradle
----
compile 'info.picocli:picocli:4.0.0-alpha-3-SNAPSHOT'
----
...
=== Scala SBT
----
libraryDependencies += "info.picocli" % "picocli" % "4.0.0-alpha-3-SNAPSHOT"
----

=== Ivy

----
<dependency org="info.picocli" name="picocli" rev="4.0.0-alpha-3-SNAPSHOT" />
----
