...
[source,bash]
----
java -cp "picocli-4.7.2.jar;myapp.jar" org.myorg.GreetingApp Sarah Lea
----
...
[source,bash]
----
java -cp "picocli-4.7.2.jar;myapp.jar" org.myorg.GreetingApp --locale=es Sarah Lea
----
...
Scripts should use `@Grab('info.picocli:picocli-groovy:4.7.2')`. The old artifact id `@Grab('info.picocli:picocli:4.7.2')` will not work,
...
Picocli has only one source file: link:https://github.com/remkop/picocli/blob/v4.7.2/src/main/java/picocli/CommandLine.java[CommandLine.java].
