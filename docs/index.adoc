...
Now in BETA: http://picocli.info/1.0.0-SNAPSHOT/autocomplete.html[command line autocompletion]. (Comments, bug reports, pull requests welcome!)
...
Then invoke `CommandLine.populateCommand` or `CommandLine.parse` with the command line parameters and an object you want to initialize.
...
Picocli has some <<Less Boilerplate,convenience methods>> that take care of handling invalid command line arguments for applications that implement `Runnable` or `Callable`.
...
==== Maps
Since 1.0.0, picocli supports `Map` fields like Java's system properties `-Dkey=value` or Gradle's project properties `-Pmyprop=myvalue`.
...
class MapDemo {
    @Option(names = {"-h", "--proxyHost"}, type = {Proxy.Type.class, InetAddress.class});
...
[source,bash]
----
<command> -hHTTP=123.123.123.123 --proxyHost SOCKS=212.212.212.212
...
Picocli 0.9.8 introduces two new option attributes, `versionHelp` and `usageHelp`.
These also stop validation to prevent "missing required option" errors, but are more specific and are preferred over the generic `help` attribute.
...
CAUTION: Picocli does not automatically print a help message.
(https://github.com/remkop/picocli/issues/161[This ticket] explains why not; feedback is welcome.)
...
Since version 0.9.8, the `CommandLine` class offers two methods that allow external components to detect whether
...
When the annotated object implements `Callable`, use the `call` method. For example:
[source,java]
----
Object result = CommandLine.call(new MyCallable(), System.err, args);
----
The `call` method returns the result of the `Callable`, or `null` if the command line options were invalid.
Exceptions thrown by the `Callable` are not caught by the `CommandLine.call` method.
...
Programmatically set separators are only used when parsing.
Changing the value does not modify the usage help message.
...
[source,bash]
----
alias git='java -Dpicocli.trace=DEBUG -cp picocli-all.jar picocli.Demo$Git'
...
[picocli DEBUG] Initializing picocli.Demo$Git: 3 options, 0 positional parameters, 0 required, 11 subcommands.
[picocli DEBUG] Processing argument '--git-dir=/home/rpopma/picocli'. Remainder=[commit, -m, "Fixed typos", --, src1.java, src2.java, src3.java]
[picocli DEBUG] Separated '--git-dir' option from '/home/rpopma/picocli' option parameter
[picocli DEBUG] Found option named '--git-dir': field java.io.File picocli.Demo$Git.gitDir, arity=1
[picocli INFO] Setting File field 'Git.gitDir' to '\home\rpopma\picocli'
[picocli DEBUG] Processing argument 'commit'. Remainder=[-m, "Fixed typos", --, src1.java, src2.java, src3.java]
[picocli DEBUG] Found subcommand 'commit' (picocli.Demo$GitCommit)
[picocli DEBUG] Initializing picocli.Demo$GitCommit: 8 options, 1 positional parameters, 0 required, 0 subcommands.
[picocli DEBUG] Processing argument '-m'. Remainder=["Fixed typos", --, src1.java, src2.java, src3.java]
[picocli DEBUG] '-m' cannot be separated into <option>=<option-parameter>
[picocli DEBUG] Found option named '-m': field java.util.List picocli.Demo$GitCommit.message, arity=0..*
[picocli INFO] Adding [Fixed typos] to List<String> field 'GitCommit.message'
[picocli DEBUG] Processing argument '--'. Remainder=[src1.java, src2.java, src3.java]
[picocli INFO] Found end-of-options delimiter '--'. Treating remainder as positional parameters
[picocli DEBUG] Processing positional parameters. Remainder=[src1.java, src2.java, src3.java]
[picocli DEBUG] Trying to assign args at index 0..* [src1.java, src2.java, src3.java] to java.util.List picocli.Demo$GitCommit.files, arity=0..*
[picocli INFO] Adding [src1.java] to List<String> field 'GitCommit.files'
[picocli INFO] Adding [src2.java] to List<String> field 'GitCommit.files'
[picocli INFO] Adding [src3.java] to List<String> field 'GitCommit.files'
----
