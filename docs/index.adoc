...
=== Customizing the Error Message On Invalid User Input

By default, when the user specifies invalid input, an error message is displayed followed by the usage help message. This can be too verbose.
Sometimes you want to display a shorter message like:
...
You can customize the response to invalid user input by setting a custom `IParameterExceptionHandler`:
