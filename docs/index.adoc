...
to do parsing and error handling in one line of code. The <<Fully Automatic Help,autoHelp>> attribute is all
...
@Command(description = "Prints the checksum (MD5 by default) of a file to STDOUT.",
         name = "checksum", autoHelp = true, version = "checksum 3.0")
...
Picocli 3.0 introduced the `autoHelp` command attribute. When this attribute is set to `true`, picocli adds a <<Mixins,mixin>> to the
...
Commands with `autoHelp` do not need to explicitly declare `usageHelp` or `versionHelp` options any more.
...
Auto-help also registers a `help` <<Usage Help for Subcommands,subcommand>> that will print help for the subcommand following it, or for this command in case no subcommand is specified. For example:
...
The `autoHelp` attribute is useful in combination with the <<Less Boilerplate,convenience methods>> that automatically print help:
...
[source,java]
----
@Command(name = "git", autoHelp = true, version = "subcommand demo - picocli 3.0",
...
The above example uses the <<Fully Automatic Help,autoHelp>> attribute to mix in
<<Help Options,`usageHelp`>> and <<Help Options,`versionHelp`>> options and register a `help` subcommand.
...
Picocli <<Fully Automatic Help,autoHelp>> internally uses a mixin.
...
[source,groovy]
----
@Grab('info.picocli:picocli:3.0.0-alpha-1-SNAPSHOT')
...
=== Gradle
----
compile 'info.picocli:picocli:3.0.0-alpha-1-SNAPSHOT'
----
...
=== Scala SBT
----
libraryDependencies += "info.picocli" % "picocli" % "3.0.0-alpha-1-SNAPSHOT"
----

=== Ivy

----
<dependency org="info.picocli" name="picocli" rev="3.0.0-alpha-1-SNAPSHOT" />
----
