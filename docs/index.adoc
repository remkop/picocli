...
Easily customizable online "command" help. Can be configured to make option matching case-insensitive,
...
public class Compress {
    @Parameters(arity = "1..*", command = "one or more files to compress")
    File[] inputFiles;

    @Option(names = {"-o", "--outfile"}, command = "optional output file name. If omitted directory name is used.")
    File outputFile;

    @Option(names = {"-h", "--help"}, help = true, command = "display this help message")
    ...
    public static void main(String... args) {
        try {
            ...
            if (compress.helpRequested) {
                CommandLine.command(compress, System.err);
                return;
            }
            doCompress();
