...
See the
https://github.com/remkop/picocli/blob/master/picocli-examples/src/main/java/picocli/examples/defaultprovider/PropertiesDefaultProvider.java[PropertiesDefaultProvider example]
for a default value provider implementation that loads default values from a
properties file named `".${command.name}.properties"` in the user home directory.
...
// TODO When creating new sections, the `Help.createHeading(String, Object...)` and `Help.createTextTable(Map<?, ?>)` methods may be useful.
