...
but an alternative is to provide a subcommand that generates the completion script for the top-level comand.
That allows users to source the completion script like this:
...
Below is an example of what a subcommand that generates a completion script could look like.
...
@Command(name = "generate-completion",
        description = "Generate a completion script for ${PARENT-COMMAND-NAME}")
...
See also this https://github.com/remkop/picocli/blob/master/picocli-examples/src/main/java/picocli/examples/autocomplete/CommandWithCompletionSubcommand.java[example].
